...
Player.prototype.act = function(step, level, keys) {
  ...
  if (level.status == "squished")
  { //this.pos.y -= step; //what does this do?
    this.size = (new Vector(1.5, .2));
  }
};
...
// Arrow key codes for readibility NOW USING A & D with Space bar
var arrowCodes = {65: "left", 32: "up", 68: "right"};
...
function runGame(plans, Display) {
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost" || status == "squished")
        startLevel(n);
